...
import { emptySong, normalizeSong } from '../model/song';
...
export function readDraft(id: string): DraftChart | null {
  ...
  if (!raw) return null;
  try {
    const parsed = JSON.parse(raw) as DraftChart;
    // Drafts/current autosaves written before loader 0.4.1 carry no ListArtist/Icon. Normalize
    // here (same coercion as zip import) so a revived draft can't export a song.json missing a
    // field the loader rejects.
    return { ...parsed, song: normalizeSong(parsed.song ?? emptySong()) };
  } catch {
    return null;
  }
}
