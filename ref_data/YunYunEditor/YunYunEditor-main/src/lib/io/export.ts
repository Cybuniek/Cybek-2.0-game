...
export interface ExportInput {
  ...
  // Optional cosmetic asset — null when the song has no Icon.
  icon: { filename: string; bytes: ArrayBuffer } | null;
...
export async function buildZip(input: ExportInput): Promise<ExportResult | ExportError> {
  ...
  // song.Icon is always emitted (even as "") by stringifySong; only the bytes are conditional.
  if (input.icon) {
    dir.file(input.song.Icon || input.icon.filename, input.icon.bytes);
  }
