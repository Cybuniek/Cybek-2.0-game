...
// Icon is intentionally absent — it's optional and a "" value is valid on export.
const REQUIRED_SONG_FIELDS: (keyof SongJson)[] = [
  'ID', 'Audio', 'Title', 'Artist', 'ListArtist', 'Lyricist', 'Composer', 'Arranger', 'Levels',
];
...
export function validateForExport(
  ...
  // Icon is optional: only constrain it when set. It shares the zip with song.json, the level
  // files, and the audio — a colliding path would silently overwrite one of them.
  if (song.Icon) {
    if (!isSafeRelativeFilename(song.Icon)) {
      issues.push({
        severity: 'error',
        message: `icon filename "${song.Icon}" is not a safe relative filename`,
      });
    } else if (
      !hasExtension(song.Icon, '.png') &&
      !hasExtension(song.Icon, '.jpg') &&
      !hasExtension(song.Icon, '.jpeg')
    ) {
      issues.push({ severity: 'error', message: `icon filename "${song.Icon}" must end with .png, .jpg, or .jpeg` });
    } else if (song.Icon.toLowerCase() === 'song.json') {
      issues.push({ severity: 'error', message: `icon filename collides with song.json` });
    } else if (seenPaths.has(song.Icon.toLowerCase())) {
      issues.push({ severity: 'error', message: `icon filename "${song.Icon}" collides with a level path` });
    } else if (song.Audio && song.Icon.toLowerCase() === song.Audio.toLowerCase()) {
      issues.push({ severity: 'error', message: `icon filename "${song.Icon}" collides with the audio filename` });
    }
  }
