...
import { normalizeSong } from '../model/song';
...
export interface ImportedMod {
  ...
  // Optional cosmetic asset — null when the mod has no Icon or its file is missing from the zip.
  icon?: { filename: string; mime: string; bytes: ArrayBuffer } | null;
  modFolderName: string;
  warnings: ValidationIssue[];
}

function iconMime(filename: string): string {
...
export async function parseZip(file: File | Blob): Promise<ImportedMod> {
  ...
  // Icon: optional. Unlike audio, a missing/absent icon must not fail the import — it's cosmetic.
  let icon: ImportedMod['icon'] = null;
  if (song.Icon) {
    const iconEntry = zip.file(baseDir + song.Icon);
    if (!iconEntry) {
      warnings.push({ severity: 'warning', message: `icon file "${song.Icon}" not found in zip` });
    } else {
      icon = {
        filename: song.Icon,
        mime: iconMime(song.Icon),
        bytes: await iconEntry.async('arraybuffer'),
      };
    }
  }
...
export function parseSongJson(text: string): SongJson {
  ...
  // Construct an explicit SongJson rather than passing raw through; normalizeSong applies the
  // lenient 0.4.1-field coercion (ListArtist/Icon → "" when missing, since pre-0.4.1 mods omit
  // them) — shared with the draft-load path so the rule lives in one place.
  return normalizeSong({
    ID: raw.ID as string,
    Audio: raw.Audio as string,
    Title: raw.Title as string,
    Artist: raw.Artist as string,
    ListArtist: raw.ListArtist as string,
    Lyricist: raw.Lyricist as string,
    Composer: raw.Composer as string,
    Arranger: raw.Arranger as string,
    Icon: raw.Icon as string,
    Levels: raw.Levels as unknown as SongJson['Levels'],
  });
}
