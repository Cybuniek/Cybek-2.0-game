<script lang="ts">
  ...
  async function saveAs() {
    ...
    try {
      ...
      // Icon bytes follow the same rule — copy alongside audio so the draft is self-contained.
      const curIcon = await getImage(CURRENT_ID);
      if (curIcon) await putImage({ ...curIcon, id });
    } catch (err: any) {
  ...
  async function load(id: string) {
    ...
    let icon: Awaited<ReturnType<typeof getImage>>;
    try {
      audio = await getAudio(id);
      icon = await getImage(id);
    } catch (err: any) {
    ...
    try {
      ...
      if (icon) {
        await putImage({ ...icon, id: CURRENT_ID });
      } else {
        await deleteImage(CURRENT_ID);
      }
    } catch (err: any) {
